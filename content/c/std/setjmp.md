...
+++
title = "<setjmp.h>"
date = 2025-04-24T18:33:23+08:00
weight = 110
type = "docs"
description = "非局部跳转"
isCJKLanguage = true
...
- C23 标准（ISO/IEC 9899:2024）：
  ...
  - 7.13/2 Nonlocal jumps <setjmp.h> （第 TBD 页）
...
| **jmp_buf** 的 **[C++ 文档](https://zh.cppreference.com/w/cpp/utility/program/jmp_buf)** |                       |
...
​	若在调用 [setjmp](https://zh.cppreference.com/w/c/program/setjmp) 时，[VLA](https://zh.cppreference.com/w/c/language/array) 或其他[可变修改类型](https://zh.cppreference.com/w/c/language/declarations)的变量在作用域中，并且控制流离开了该作用域，则 `longjmp` 到该 `setjmp` 将引发未定义行为，即使控制流仍然留在该函数内。 ​	在上溯栈的途中，`longjmp` 不会解分配任何 VLA，若其的生存期以此方式终结，则会出现内存泄漏： (C99 起)
...
| [setjmp<br />](https://zh.cppreference.com/w/c/program/setjmp) | 保存上下文 (宏函数) |
| ------------------------------------------------------------ | ------------------- |
| **longjmp** 的 **[C++ 文档](https://zh.cppreference.com/w/cpp/utility/program/longjmp)** |                     |
...
1. 选择或迭代语句（[`<>`](https://zh.cppreference.com/w/c/language/if)、[`<witc>`](https://zh.cppreference.com/w/c/language/switch)、[`<hil>`](https://zh.cppreference.com/w/c/language/while)、[`<o-whil>`](https://zh.cppreference.com/w/c/language/do)、[`<o>`](https://zh.cppreference.com/w/c/language/for)）的完整控制表达式
...
- 但含有 `setjmp` 调用的函数中的非 [`<olatil>`](https://zh.cppreference.com/w/c/language/volatile) 局部变量，若已在 `setjmp` 调用后有所更改，则其值不确定。
...
- C17 标准（ISO/IEC 9899:2018）：
  ...
  - 7.13.1.1 The setjmp macro （第 191 页）

- C11 标准（ISO/IEC 9899:2011）：

  - 7.13.1.1 The setjmp macro （第 262-263 页）

- C99 标准（ISO/IEC 9899:1999）：

  - 7.13.1.1 The setjmp macro （第 243-244 页）
...
| [longjmp<br />](https://zh.cppreference.com/w/c/program/longjmp) | 跳转到指定位置 (函数) |
| ------------------------------------------------------------ | --------------------- |
| **setjmp** 的 **[C++ 文档](https://zh.cppreference.com/w/cpp/utility/program/setjmp)** |                       |
