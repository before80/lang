...
（内建类型上的更多细节参阅[算术类型]({{< ref "/c/language/basic_concepts/arithmetic_types" >}})和 C 库所提供的[类型相关的工具列表]({{< ref "/c/types" >}})。）

​	[对象]({{< ref "/c/language/basic_concepts/object" >}})、[函数]({{< ref "/c/language/functions" >}})和[表达式]({{< ref "/c/language/expressions" >}})拥有称为*类型* ﻿的属性，它确定存储于对象或表达式求值所得的二进制值的判读方式。
...
- [枚举类型]({{< ref "/c/language/declarations/enum" >}})

- 派生类型
  - [数组类型]({{< ref "/c/language/declarations/array" >}})
  - [结构体类型]({{< ref "/c/language/declarations/struct" >}})
  - [联合体类型]({{< ref "/c/language/declarations/union" >}})
  - [函数类型]({{< ref "/c/language/functions" >}})
  - [指针类型]({{< ref "/c/language/declarations/pointer" >}})
  - [原子类型]({{< ref "/c/language/declarations/atomic" >}})(C11 起)

​	对于上面列出的每个类型，可以存在数种其类型的限定版本，对应 [`const`]({{< ref "/c/language/declarations/const" >}})、[`volatile`]({{< ref "/c/language/declarations/volatile" >}}) 和 [`restrict`]({{< ref "/c/language/declarations/restrict" >}}) 限定符的一、二或全部三个组合（在限定符的语义所允许处）。
...
- *实数类型*：整数类型和实浮点数类型
- [*算术类型*]({{< ref "/c/language/basic_concepts/arithmetic_types" >}})：整数类型和浮点数类型
- *标量类型*：算术类型和指针类型以及 [nullptr_t]({{< ref "/c/types/nullptr_t" >}})(C23 起)
- *聚合类型*：数组类型和结构体类型
- *派生声明符类型*：数组类型、函数类型和指针类型

​	构造一个完整对象类型，使其对象表示中的字节数不能以 [size_t]({{< ref "/c/types/size_t" >}}) 类型（即 [`sizeof`]({{< ref "/c/language/expressions/sizeof" >}}) 运算符的结果类型）表示，包括在运行时构成这种 VLA 类型，(C99 起)是未定义行为。
...
- 它们是同一类型（同名或由 [`typedef`]({{< ref "/c/language/declarations/typedef" >}}) 引入的别名）
...
- 它们都是结构体/联合体/枚举类型，且
  ...
  - 另外，若它们是结构体或联合体，则
    - 对应的元素必须以同一顺序声明（仅结构体）
    - 对应的[位域]({{< ref "/c/language/declarations/bit_field" >}})必须有相同宽度。
...
​	对于拥有内部或外部[链接]({{< ref "/c/language/declarations/storage_duration" >}})，并在其先前声明已经可见的作用域中再次声明的标识符，若先前的声明指定了内部或外部链接，则在后一声明中的标识符类型成为合成类型。
...
​	在除[声明]({{< ref "/c/language/declarations" >}})之外的语境中可能需要指名某个类型。这些情况下使用*类型名*，即*类型说明符* ﻿和*类型限定符* ﻿的列表，后随*声明符*（见[声明]({{< ref "/c/language/declarations" >}})），它在文法上与会用来声明此类型的单个对象或函数的列表完全相同，除了省略掉其中的标识符：
...
- [转换]({{< ref "/c/language/expressions/cast" >}})
- [`sizeof`]({{< ref "/c/language/expressions/sizeof" >}})
- [复合字面量]({{< ref "/c/language/expressions/compound_literal" >}})(C99 起)
- [泛型选择]({{< ref "/c/language/expressions/generic" >}})
- [`_Alignof`]({{< ref "/c/language/expressions/_Alignof" >}})(C23 前) [`alignof`](https://zh.cppreference.com/w/c/language/alignof)(C23 起)  - (C11 起)
- [`_Alignas`]({{< ref "/c/language/declarations/_Alignas" >}})(C23 前)  [`alignas`](https://zh.cppreference.com/w/c/language/alignas)(C23 起) - (C11 起)
- [`_Atomic`]({{< ref "/c/language/declarations/atomic" >}})（用作类型说明符时） - (C11 起)
