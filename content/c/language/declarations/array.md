...
​	已知常量大小的数组可以用[数组初始化器](https://zh.cppreference.com/w/c/language/array_initialization)提供它们的初始值：
...
​	对于函数中在数组形参的 `[` 和 `]` 中使用关键词 `static` 的情况，每次对这种函数[函数调用](https://zh.cppreference.com/w/c/language/operator_other#.E5.87.BD.E6.95.B0.E8.B0.83.E7.94.A8)中，实际参数的值必须是一个指向数组首地址的合法指针，该数组至少有*表达式* ﻿所指定的元素数目：(C99 起)
...
​	这通常用于 [`restrict`](https://zh.cppreference.com/w/c/language/restrict) 类型限定符：(C99 起)
